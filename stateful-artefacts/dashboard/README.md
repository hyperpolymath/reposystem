...
└──────┬───────┘
       ...
       ▼
┌──────────────────┐
│  dashboard.js    │ ← Fetch from API
│  fetchRepoData() │
└──────┬───────────┘
       │
       ├─ GitHub API v3 (REST)
       ├─ GitLab API v4 (REST)
       └─ Bitbucket API 2.0 (REST)
       │
       ▼
┌─────────────────────┐
│  Metrics Processing │
...
├── js/
│   └── dashboard.js    # Core logic and API calls
└── README.md           # This file
```
...
✅ **Token storage** - Tokens used only for API requests, not stored
✅ **CORS-friendly** - Works with public APIs
