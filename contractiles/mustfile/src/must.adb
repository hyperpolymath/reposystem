   ...
   begin
      case Args.Command is
         ...
         when Cmd_Run_Task =>
            ...
            Config := Mustfile_Loader.Load;
            declare
               Task_Name : constant String := To_String (Args.Task_Name);
            begin
         ...
         when Cmd_Apply =>
            ...
            if Length (Args.Template_Name) > 0 then
               --  Apply specific template
               Mustache_Engine.Apply_Named
                 (Config        => Config,
                  Template_Name => To_String (Args.Template_Name),
         ...
         when Cmd_Deploy =>
            ...
            begin
               Deployer.Deploy
                 (Config  => Config,
                  Target  => To_String (Args.Deploy_Target),
                  Tag     => To_String (Args.Deploy_Tag),
