...
`4c2d29f`).* The "V CLI / coroutine layer" described below *was never
implemented* — git history contains no `src/v/`, no V ever existed. The real,
*built* architecture is an ATS2 verified core (L1–L4) + a self-contained
C-export archive (L5) + an Idris2 typed ABI with machine-checked theorems
(L6) + a Zig 0.15.2 CLI binding the genuine ATS2 C ABI, `patscc`-linked (L7),
behind a fixture-backed `just e2e` gate (L8). Treat every "V" reference here
as an *un-built historical plan*, not shipped code; the CLI/FFI layer is Zig
per the estate API/FFI standard. reposystem#56 is CLOSED-COMPLETED.
====
...
* **Parallel Execution**: _(Phase 3, not yet implemented)_ multi-repository concurrency will live in the Zig FFI/CLI layer (now built — L7); parallelism itself is future work (reposystem#56 Phase 3). No V was ever used.
...
----
┌─────────────────────────────────────┐
│  Zig FFI/CLI Layer  [IMPLEMENTED L7]  │  CLI built; binds ATS2 C ABI, patscc-linked
│  (ffi/zig/ + src/ats2/ffi/cli_root)   │  (reposystem#56 PR#59; no src/v/ ever existed)
└──────────────┬──────────────────────┘
               │ FFI
┌──────────────▼──────────────────────┐
...
| *No V was ever used* (despite older prose). The CLI/FFI layer is Zig (estate API/FFI standard), implemented in L7: it binds the genuine ATS2 C ABI and is `patscc`-linked into a working binary, fixture-gated by `just e2e` (L8). Delivered in reposystem#56 PR #59 (merged 2026-05-18, `4c2d29f`).
...
[replacements]
old_pattern = '''authors = \[".*"\]'''
new_value = '''authors = ["Jonathan D.A. Jewell <j.d.a.jewell@open.ac.uk>"]'''
