...
package body Task_Runner is
   ...
   begin
      for T of Config.Tasks loop
         if To_String (T.Name) = Task_Name then
            return True;
         end if;
      end loop;
      return False;
   ...
   begin
      ...
      if Length (T.Working_Dir) > 0 then
         if Verbose then
            Put_Line ("  cd " & To_String (T.Working_Dir));
         end if;
         if not Dry_Run then
            Ada.Directories.Set_Directory (To_String (T.Working_Dir));
         end if;
      end if;
      ...
      if Length (T.Script) > 0 then
         ...
         if not Dry_Run then
            declare
               Status : Integer;
            begin
               Status := Execute_Command (To_String (T.Script), Verbose);
               if Status /= 0 then
                  raise Task_Error with
   ...
   begin
      ...
      for T of Config.Tasks loop
         declare
            ...
            Desc : constant String := To_String (T.Description);
